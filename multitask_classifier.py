...
class MultitaskBERT(nn.Module):
    ...
    def predict_paraphrase(self,
        ...
        embeddings_sim = embeddings_1 * embeddings_2
        concat_embeddings = torch.cat((embeddings_diff, embeddings_sim), dim=-1)
        concat_embeddings = self.dropout(concat_embeddings)
        logits = self.paraphrase_classifier(concat_embeddings)
        return logits
    ...
    def predict_similarity(self,
        ...
        cosine_similarity = F.cosine_similarity(embeddings_1, embeddings_2)
        
        return 5 * cosine_similarity
...
def train_multitask(args):
    ...
    limited_para_train_data = para_train_data[:60000]
    ...
    for epoch in range(args.epochs):
        ...
        progress_bar = tqdm(total=len(sst_train_dataloader) + len(para_train_dataloader) + len(sts_train_dataloader),
        ...
        for sst_batch, para_batch, sts_batch in zip(sst_train_dataloader, para_train_dataloader, sts_train_dataloader):
            ...
            task_losses["SST"] = F.cross_entropy(sst_logits, b_labels.view(-1))

            b_ids_1, b_mask_1, b_ids_2, b_mask_2, b_labels = (para_batch['token_ids_1'], para_batch['attention_mask_1'], para_batch['token_ids_2'], para_batch['attention_mask_2'], para_batch['labels'])
            ...
            task_losses["Paraphrase"] = F.binary_cross_entropy_with_logits(para_logits.view(-1), b_labels.view(-1).float())

            b_ids_1, b_mask_1, b_ids_2, b_mask_2, b_labels = (sts_batch['token_ids_1'], sts_batch['attention_mask_1'], sts_batch['token_ids_2'], sts_batch['attention_mask_2'], sts_batch['labels'])
            ...
            task_losses["STS"] = F.mse_loss(sts_logits, b_labels.view(-1).to(sts_logits.dtype))
        ...
        train_loss = total_train_loss / total_num_batches
